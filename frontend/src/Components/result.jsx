  ...
  return (
    ...
    >
      ...
      <div className="bg-white dark:bg-gray-800 p-6 border-b border-gray-200 dark:border-gray-700">
        ...
        <div className="mb-6" aria-label={`Confidence level is ${confidencePercent} percent`}>
          <div className="flex justify-between items-center mb-3">
            <div className="flex items-center">
              ...
              <span className="font-bold text-lg text-gray-800 dark:text-gray-200">Confidence Level</span>
            </div>
          ...
          <div className="relative pt-1">
            <div className="w-full h-6 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden" role="progressbar" aria-valuenow={confidencePercent} aria-valuemin={0} aria-valuemax={100}>
              <div
                className={`h-full ${
                  isReal 
            ...
            <div className="flex justify-between mt-1 px-1 text-xs text-gray-600 dark:text-gray-400">
              <span>Low</span>
              <span>Moderate</span>
              <span>High</span>
            </div>
        ...
        {/* Analysis details */}
        <div className="mb-6 text-gray-800 dark:text-gray-200 text-base leading-relaxed">
          <div className={`p-4 mb-4 border-l-4 ${isReal ? 'border-green-500 bg-green-50 dark:bg-green-900 dark:border-green-700' : 'border-red-500 bg-red-50 dark:bg-red-900 dark:border-red-700'} rounded-r-lg`}>
            <p className="font-medium text-gray-800 dark:text-gray-200">{result.message}</p>
          </div>
          
          <p className={`mt-3 font-medium ${isReal ? 'text-green-700 dark:text-green-300' : 'text-red-700 dark:text-red-300'}`}>
            {isReal
        ...
        <section
          className={`mb-6 p-5 rounded-lg shadow-sm ${
            isReal 
              ? 'bg-gradient-to-br from-green-50 to-green-100 dark:from-green-900 dark:to-green-800 border border-green-200 dark:border-green-700' 
              : 'bg-gradient-to-br from-red-50 to-red-100 dark:from-red-900 dark:to-red-800 border border-red-200 dark:border-red-700'
          }`}
          aria-label={isReal ? 'Verification tips' : 'Warning signs'}
        >
          <div className="flex items-start">
            <div className={`rounded-full p-2 mr-3 flex-shrink-0 ${isReal ? 'bg-green-200 dark:bg-green-800' : 'bg-red-200 dark:bg-red-800'}`}>
              <svg xmlns="http://www.w3.org/2000/svg" className={`h-5 w-5 ${isReal ? 'text-green-700 dark:text-green-300' : 'text-red-700 dark:text-red-300'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
            </div>
            <div>
              <h4 className={`text-lg font-semibold mb-2 ${isReal ? 'text-green-800 dark:text-green-200' : 'text-red-800 dark:text-red-200'}`}>
      ...
      <div className={`px-6 py-4 ${isReal ? 'bg-green-50 dark:bg-green-900' : 'bg-red-50 dark:bg-red-900'}`}>
        <button
          onClick={onReset}
